...
const HeaderThree = () => {
  ...
  const handleClick = (path: string, onClose: () => void) => {
    navigate(path);
    onClose();
  };
  return (
          ...
          <Menu>
            {({ onClose }) => (
              <>
                ...
                <MenuList className=" rounded-xl overflow-hidden  shadow-md [&>ul>li]:text-white [&>ul>li]:text-sm [&>ul>li]:bg-dark-accent-1 [&>ul>li>button:hover]:text-orange-accent-3   !p-0  ">
                  <ul className="flex flex-col text-dark [&>li]:pl-2 py-2 gap-y-3  text-xs bg-dark-accent-1 ">
                    {thirdHeader?.map((item, idx) => (
                      <li key={idx}>
                        <button onClick={() => handleClick(item.path, onClose)}>
                          {item.name}{" "}
                        </button>
                      </li>
                    ))}
